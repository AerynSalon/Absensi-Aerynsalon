  ...
  try{
    ...
    if (hex){
      ...
      function darken(h){
        ...
        root.setProperty("--brand-hover", "#"+toHex(r)+toHex(g)+toHex(b));
      }
      darken(hex);
    }
  }catch(e){}
...
// === Admin role helpers + login fallback ===
...
async function loginAdmin(){
  ...
  const defaultPw = "aeryn123";
  const storedHash = localStorage.getItem("adminPassHash");
  const storedPlain = localStorage.getItem("adminPassPlain");
  let ok = false;
  if (storedPlain){
    ok = pass === storedPlain;
  } else {
    const hInput = await sha256Hex(pass);
    if (hInput){
      const hDefault = await sha256Hex(defaultPw);
      ok = (storedHash ? storedHash : hDefault) === hInput;
    } else {
      ok = pass === defaultPw;
    }
  }
